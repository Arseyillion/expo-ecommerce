...
const NewArrivalsList = () => {
  const { data, isLoading, isError } = useNewArrivals(8);

  if (isLoading) return <div>Loading...</div>;
  if (isError || !data) return <div>Failed to load new arrivals.</div>;

  // Map backend product shape to the ProductItem shape used by the template
  ...
  return (
    <>
      {mapped.map((item: any, idx: number) => (
        <ProductItem item={item} key={item.id || idx} />
      ))}
    </>
  );
};
